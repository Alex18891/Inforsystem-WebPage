...
export default function Drives() {
    ...
    const [drivesfilter, setdrivesfilter] = useState([]);
    const [maxpages, setmaxpages] = useState([]);
    const [maxpagesfilter, setmaxpagesfilter] = useState([]);
    const [marcadrives, setmarcadrives] = useState([]);
    const [checkboxmarca,setcheckboxmarca] = useState(Array(marcadrives.length).fill(false));
    const pageNumber = queryParams.get("page");
    const itemsToShow = drives.slice(((parseInt(pageNumber, 10) ) - 1) * itemsPerPage, (parseInt(pageNumber, 10) ) * itemsPerPage);
    ...
    const renderLinks = () => {
        ...
        for (let i = 0; i < maxpages; i++) {
          ...
          if (currentNumber <= maxpages && currentNumber <= 7) {
            elements.push(
              <Link to={`/drives?page=${currentNumber}`} id='aheader' key={currentNumber}>
                {currentNumber}
                <Text>&nbsp; | </Text>
              </Link>
            );
    ...
    const readFile = async () => {
        ...
        reader.onload = (e) => {
            ...
            if (jsonData && jsonData.length > 0) {
                ...
                const combinedmarcaarray =Array.from(new Set(drives.map(value => value[0]))) 
                setmarcadrives(combinedmarcaarray)
                const maxPages = Math.ceil(drives.length / itemsPerPage);
                setmaxpages(maxPages)
                setdrives(drives)
                setdrivesfilter(drives)
    ...
    useEffect(()=>{       
        const filterBySelectedCheckboxes = () => {
            return drivesfilter.filter(item=>{      //Filter the pcs by family     
                for(let i = 0; i<checkboxmarca.length;i++)//For that runs up to all the checkboxs
                {
                    console.log( marcadrives[i])
                    if(checkboxmarca[i] && item[0] ==  marcadrives[i])//If the checkbox is selecte and element 1 of pcsfilter array(family) is equal to the familypcs array return true
                    {    
                        return true;
                    }   
                }
    ...
    },[checkboxmarca, marcadrives, drivesfilter])
            ...
            <Box sx={{maxWidth:"1800px",  margin:"auto",paddingLeft:"0.5rem",paddingRight:"0.5rem"}}>
                ...
                ...(isExtraSmallScreen && styles.containerextrasmall),
                    ...
                    <Text style={{
                        ...
                        <span style={{fontWeight:"bold"}}>Drives</span>
                    </Text>
                    <Text style={{
                        ...styles.textdefault,
                        maxWidth: "1800px",
                        ...
                        Veja as drives disponíveis na loja
                    </Text>
                </Box>
                <Box sx={{...styles.containermain, 
                    ...
                    ...(isExtraSmallScreen && styles.container1extrasmall)}}>
                        {drives.length>0  &&(
                                itemsToShow.map((drives, index) => (
                                        <Box sx={styles.viewcontainer} onClick = {(e) =>handleproduct(drives)}>
                                        <Box sx={styles.containerfeaturesmainproduct}> 
                                            ...
                                            <Box sx={styles.containerfeatures}>
                                                ...
                                                <Text style={[styles.textdefault,{fontSize:"13px"}]}>Ref: {drives[2]} </Text>    
                                                ...
                                                <Text style={styles.textdefault2}>
                                                    <span style={{color:"black"}}>{drives[5]} €</span> 
                                                </Text>
                                            </Box>
                                        </Box>
                                        </Box> 
                        ...
                        <Box sx={styles.pages}>
                            <Box sx={styles.pagesflex}>      
                                {parseInt(pageNumber, 10) <= maxpages && parseInt(pageNumber, 10) > 1 && (
                                    <Link  to={`/drives?page=${parseInt(pageNumber, 10) - 1}`} id='aheader' >
