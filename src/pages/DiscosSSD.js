...
export default function DiscosSSD() {
    ...
    const [discosssd, setdiscosssd] = useState([]);
    const [discosssdfilter, setdiscosssdfilter] = useState([]);
    const [maxpages, setmaxpages] = useState([]);
    const [maxpagesfilter, setmaxpagesfilter] = useState([]);
    const [marcadiscosssd, setmarcadiscosssd] = useState([]);
    const [checkboxmarca,setcheckboxmarca] = useState(Array(marcadiscosssd.length).fill(false));
    const pageNumber = queryParams.get("page");
    const itemsToShow = discosssd.slice(((parseInt(pageNumber, 10) ) - 1) * itemsPerPage, (parseInt(pageNumber, 10) ) * itemsPerPage);
    ...
    const renderLinks = () => {
        ...
        for (let i = 0; i < maxpages; i++) {
          ...
          if (currentNumber <= maxpages && currentNumber <= 7) {
            elements.push(
              <Link to={`/discosSSD?page=${currentNumber}`} id='aheader' key={currentNumber}>
                {currentNumber}
                <Text>&nbsp; | </Text>
              </Link>
            );
    ...
    const readFile = async () => {
        ...
        reader.onload = (e) => {
            ...
            if (jsonData && jsonData.length > 0) {
                ...
                const combinedmarcaarray =Array.from(new Set(discosssd.map(value => value[0]))) 
                setmarcadiscosssd(combinedmarcaarray)
                const maxPages = Math.ceil(discosssd.length / itemsPerPage);
                setmaxpages(maxPages)
                setdiscosssd(discosssd)
                setdiscosssdfilter(discosssd)
    ...
    useEffect(()=>{       
        const filterBySelectedCheckboxes = () => {
            return discosssdfilter.filter(item=>{      //Filter the pcs by family     
                for(let i = 0; i<checkboxmarca.length;i++)//For that runs up to all the checkboxs
                {
                    console.log( marcadiscosssd[i])
                    if(checkboxmarca[i] && item[0] ==  marcadiscosssd[i])//If the checkbox is selecte and element 1 of pcsfilter array(family) is equal to the familypcs array return true
                    {    
                        return true;
                    }   
                }
        ...
        if(deduplicated.length>0)
        {
            ...
            setmaxpages(maxPages);
            setdiscosssd(deduplicated);
            navigate('?page=1');
        }
        else{
            setdiscosssd(discosssdfilter);
            setmaxpages(maxpagesfilter);
            navigate('?page=1');
        }
      
    },[checkboxmarca, marcadiscosssd, discosssdfilter])
            ...
            <Box sx={{maxWidth:"1800px",  margin:"auto",paddingLeft:"0.5rem",paddingRight:"0.5rem"}}>
                ...
                ...(isExtraSmallScreen && styles.containerextrasmall),
                    ...
                    <Box>
                        ...
                        <Text  style={{fontSize: "20px",zIndex:-1}}>Discos SSD</Text>   
                        </Box>
                    </Box>     
                    <Text style={{
                        ...styles.textdefault3,
                        ...
                        <span style={{fontWeight:"bold"}}>Discos SSD</span>
                    </Text>
                    <Text style={{
                        ...styles.textdefault,
                        maxWidth: "1800px",
                        ...
                        Veja os discos ssd disponíveis na loja
                    </Text>
                </Box>
                                ...
                                itemsToShow.map((discosssd, index) => (
                                        ...
                                        <Box sx={styles.containerfeaturesmainproduct}> 
                                            ...
                                            <Box sx={styles.containerfeatures}>
                                                ...
                                                <Text style={[styles.textdefault,{fontSize:"13px"}]}>Ref: {discosssd[2]} </Text>    
                                                ...
                                                <Text style={styles.textdefault2}>
                                                    <span style={{color:"black"}}>{discosssd[5]} €</span> 
                                                </Text>
                                            </Box>
                                        </Box>
                                        </Box> 
                        ...
                        <Box sx={styles.pages}>
                            <Box sx={styles.pagesflex}>
                                {parseInt(pageNumber, 10) <= maxpages && parseInt(pageNumber, 10) > 1 && (
                                    <Link  to={`/discosSSD?page=${parseInt(pageNumber, 10) - 1}`} id='aheader' >
