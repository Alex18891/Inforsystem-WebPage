...
export default function PosImpressoras() {
    ...
    const [posimpressoras, setposimpressoras] = useState([]);
    const [posimpressorasfilter, setposimpressorasfilter] = useState([]);
    const [maxpages, setmaxpages] = useState([]);
    const [maxpagesfilter, setmaxpagesfilter] = useState([]);
    const [marcaposimpressoras, setmarcaposimpressoras] = useState([]);
    const [checkboxmarca,setcheckboxmarca] = useState(Array(marcaposimpressoras.length).fill(false));
    const pageNumber = queryParams.get("page");
    const itemsToShow = posimpressoras.slice(((parseInt(pageNumber, 10) ) - 1) * itemsPerPage, (parseInt(pageNumber, 10) ) * itemsPerPage);
    ...
    const renderLinks = () => {
        ...
        for (let i = 0; i < maxpages; i++) {
          ...
          if (currentNumber <= maxpages && currentNumber <= 7) {
            elements.push(
              <Link to={`/posImpressoras?page=${currentNumber}`} id='aheader' key={currentNumber}>
                {currentNumber}
                <Text>&nbsp; | </Text>
              </Link>
            );
    ...
    const readFile = async () => {
        ...
        reader.onload = (e) => {
            ...
            if (jsonData && jsonData.length > 0) {
                ...
                const combinedmarcaarray =Array.from(new Set(Posimpre.map(value => value[0]))) 
                setmarcaposimpressoras(combinedmarcaarray)
                const maxPages = Math.ceil(Posimpre.length / itemsPerPage);
                setmaxpages(maxPages)
                setposimpressoras(Posimpre)
                setposimpressorasfilter(Posimpre)
    ...
    useEffect(()=>{       
        const filterBySelectedCheckboxes = () => {
            return posimpressorasfilter.filter(item=>{      //Filter the pcs by family     
                for(let i = 0; i<checkboxmarca.length;i++)//For that runs up to all the checkboxs
                {
                    console.log( marcaposimpressoras[i])
                    if(checkboxmarca[i] && item[0] ==  marcaposimpressoras[i])//If the checkbox is selecte and element 1 of pcsfilter array(family) is equal to the familypcs array return true
                    {    
                        return true;
                    }   
                }
        ...
        if(deduplicated.length>0)
        {
            ...
            setmaxpages(maxPages);
            setposimpressoras(deduplicated);
            navigate('?page=1');
        }
        else{
            setposimpressoras(posimpressorasfilter);
            setmaxpages(maxpagesfilter);
            navigate('?page=1');
        }
      
    },[checkboxmarca, marcaposimpressoras, posimpressorasfilter])
        ...
        <Box  sx={styles.container1}>
            ...
            <Box sx={styles.containerfeaturesmainproduct}> 
               ...
               <Box sx={styles.containerfeatures}>
                   {
                      posimpressoras.length>0  &&(
                           marcaposimpressoras.map((pc, index) => (
            ...
            <Box sx={{maxWidth:"1800px",  margin:"auto",paddingLeft:"0.5rem",paddingRight:"0.5rem"}}>
                ...
                ...(isExtraSmallScreen && styles.containerextrasmall),
                    ...
                    <Box>
                        ...
                        <Text  style={{fontSize: "20px",zIndex:-1}}>Pos Impressoras</Text>   
                        </Box>
                    </Box>     
                    <Text style={{
                        ...styles.textdefault3,
                    ...
                    <Text style={{
                        ...
                        Veja os pos impressoras disponíveis na loja
                    </Text>
                </Box>
                <Box sx={{...styles.containermain, 
                    ...
                    ...(isExtraSmallScreen && styles.container1extrasmall)}}>
                        {posimpressoras.length>0  &&(
                                itemsToShow.map((posimpressoras, index) => (
                                        <Box sx={styles.viewcontainer} onClick = {(e) =>handleproduct(posimpressoras)}>
                                        <Box sx={styles.containerfeaturesmainproduct}> 
                                            ...
                                            <Box sx={styles.containerfeatures}>
                                                ...
                                                <Text style={[styles.textdefault,{fontSize:"13px"}]}>Ref: {posimpressoras[2]} </Text>    
                                                ...
                                                <Text style={styles.textdefault2}>
                                                    <span style={{color:"black"}}>{posimpressoras[5]} €</span> 
                                                </Text>
                                            </Box>
                                        </Box>
                                        </Box> 
                        ...
                        <Box sx={styles.pages}>
                            <Box sx={styles.pagesflex}>
                                {parseInt(pageNumber, 10) <= maxpages && parseInt(pageNumber, 10) > 1 && (
                                    <Link  to={`/posImpressoras?page=${parseInt(pageNumber, 10) - 1}`} id='aheader' >
