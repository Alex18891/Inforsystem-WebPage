...
export default function Posmonitores() {
    ...
    const [posmonitores, setposmonitores] = useState([]);
    const [posmonitoresfilter, setposmonitoresfilter] = useState([]);
    const [maxpages, setmaxpages] = useState([]);
    const [maxpagesfilter, setmaxpagesfilter] = useState([]);
    const [marcaposmonitores, setmarcaposmonitores] = useState([]);
    const [checkboxmarca,setcheckboxmarca] = useState(Array(marcaposmonitores.length).fill(false));
    const pageNumber = queryParams.get("page");
    const itemsToShow = posmonitores.slice(((parseInt(pageNumber, 10) ) - 1) * itemsPerPage, (parseInt(pageNumber, 10) ) * itemsPerPage);
    ...
    const renderLinks = () => {
        ...
        for (let i = 0; i < maxpages; i++) {
          ...
          if (currentNumber <= maxpages && currentNumber <= 7) {
            elements.push(
              <Link to={`/posmonitores?page=${currentNumber}`} id='aheader' key={currentNumber}>
                {currentNumber}
                <Text>&nbsp; | </Text>
              </Link>
            );
    ...
    const readFile = async () => {
        ...
        reader.onload = (e) => {
            ...
            if (jsonData && jsonData.length > 0) {
                ...
                const combinedmarcaarray =Array.from(new Set(Posmonito.map(value => value[0]))) 
                setmarcaposmonitores(combinedmarcaarray)
                const maxPages = Math.ceil(Posmonito.length / itemsPerPage);
                setmaxpages(maxPages)
                setposmonitores(Posmonito)
                setposmonitoresfilter(Posmonito)
    ...
    useEffect(()=>{       
        const filterBySelectedCheckboxes = () => {
            return posmonitoresfilter.filter(item=>{      //Filter the pcs by family     
                for(let i = 0; i<checkboxmarca.length;i++)//For that runs up to all the checkboxs
                {
                    console.log( marcaposmonitores[i])
                    if(checkboxmarca[i] && item[0] ==  marcaposmonitores[i])//If the checkbox is selecte and element 1 of pcsfilter array(family) is equal to the familypcs array return true
                    {    
                        return true;
                    }   
                }
        ...
        if(deduplicated.length>0)
        {
            ...
            setmaxpages(maxPages);
            setposmonitores(deduplicated);
            navigate('?page=1');
        }
        else{
            setposmonitores(posmonitoresfilter);
            setmaxpages(maxpagesfilter);
            navigate('?page=1');
        }
      
    },[checkboxmarca, marcaposmonitores, posmonitoresfilter])
        ...
        <Box  sx={styles.container1}>
            ...
            <Box sx={styles.containerfeaturesmainproduct}> 
               ...
               <Box sx={styles.containerfeatures}>
                   {
                      posmonitores.length>0  &&(
                           marcaposmonitores.map((pc, index) => (
            ...
            <Box sx={{maxWidth:"1800px",  margin:"auto",paddingLeft:"0.5rem",paddingRight:"0.5rem"}}>
                ...
                ...(isExtraSmallScreen && styles.containerextrasmall),
                    ...
                    <Box>
                        ...
                        <Text  style={{fontSize: "20px",zIndex:-1}}>Pos Monitores</Text>   
                        </Box>
                    </Box>     
                    <Text style={{
                        ...styles.textdefault3,
                        ...
                        <span style={{fontWeight:"bold"}}>Pos Monitores</span>
                    </Text>
                    <Text style={{
                        ...styles.textdefault,
                        maxWidth: "1800px",
                        ...
                        Veja os pos monitores disponíveis na loja
                    </Text>
                </Box>
                <Box sx={{...styles.containermain, 
                    ...
                    ...(isExtraSmallScreen && styles.container1extrasmall)}}>
                        {posmonitores.length>0  &&(
                                itemsToShow.map((posmonitores, index) => (
                                        <Box sx={styles.viewcontainer} onClick = {(e) =>handleproduct(posmonitores)}>
                                        <Box sx={styles.containerfeaturesmainproduct}> 
                                            ...
                                            <Box sx={styles.containerfeatures}>
                                                ...
                                                <Text style={[styles.textdefault,{fontSize:"13px"}]}>Ref: {posmonitores[2]} </Text>    
                                                ...
                                                <Text style={styles.textdefault2}>
                                                    <span style={{color:"black"}}>{posmonitores[5]} €</span> 
                                                </Text>
                                            </Box>
                                        </Box>
                                        </Box> 
                        ...
                        <Box sx={styles.pages}>
                            <Box sx={styles.pagesflex}>
                                {parseInt(pageNumber, 10) <= maxpages && parseInt(pageNumber, 10) > 1 && (
                                    <Link  to={`/posmonitores?page=${parseInt(pageNumber, 10) - 1}`} id='aheader' >
