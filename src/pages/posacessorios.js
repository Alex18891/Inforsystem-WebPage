...
export default function Posacessorios() {
    ...
    const [posacessorios, setposacessorios] = useState([]);
    const [posacessoriosfilter, setposacessoriosfilter] = useState([]);
    const [maxpages, setmaxpages] = useState([]);
    const [maxpagesfilter, setmaxpagesfilter] = useState([]);
    const [marcaposacessorios, setmarcaposacessorios] = useState([]);
    const [checkboxmarca,setcheckboxmarca] = useState(Array(marcaposacessorios.length).fill(false));
    const pageNumber = queryParams.get("page");
    const itemsToShow = posacessorios.slice(((parseInt(pageNumber, 10) ) - 1) * itemsPerPage, (parseInt(pageNumber, 10) ) * itemsPerPage);
    ...
    const renderLinks = () => {
        ...
        for (let i = 0; i < maxpages; i++) {
          ...
          if (currentNumber <= maxpages && currentNumber <= 7) {
            elements.push(
              <Link to={`/posacessórios?page=${currentNumber}`} id='aheader' key={currentNumber}>
                {currentNumber}
                <Text>&nbsp; | </Text>
              </Link>
            );
    ...
    const readFile = async () => {
        ...
        reader.onload = (e) => {
            ...
            if (jsonData && jsonData.length > 0) {
                ...
                const combinedmarcaarray =Array.from(new Set(posacc.map(value => value[0]))) 
                setmarcaposacessorios(combinedmarcaarray)
                const maxPages = Math.ceil(posacc.length / itemsPerPage);
                setmaxpages(maxPages)
                setposacessorios(posacc)
                setposacessoriosfilter(posacc)
    ...
    useEffect(()=>{       
        const filterBySelectedCheckboxes = () => {
            return posacessoriosfilter.filter(item=>{      //Filter the pcs by family     
                for(let i = 0; i<checkboxmarca.length;i++)//For that runs up to all the checkboxs
                {
                    console.log( marcaposacessorios[i])
                    if(checkboxmarca[i] && item[0] ==  marcaposacessorios[i])//If the checkbox is selecte and element 1 of pcsfilter array(family) is equal to the familypcs array return true
                    {    
                        return true;
                    }   
                }
        ...
        if(deduplicated.length>0)
        {
            ...
            setmaxpages(maxPages);
            setposacessorios(deduplicated);
            navigate('?page=1');
        }
        else{
            setposacessorios(posacessoriosfilter);
            setmaxpages(maxpagesfilter);
            navigate('?page=1');
        }
      
    },[checkboxmarca, marcaposacessorios, posacessoriosfilter])
        ...
        <Box  sx={styles.container1}>
            ...
            <Box sx={styles.containerfeaturesmainproduct}> 
               ...
               <Box sx={styles.containerfeatures}>
                   {
                      posacessorios.length>0  &&(
                           marcaposacessorios.map((pc, index) => (
            ...
            <Box sx={{maxWidth:"1800px",  margin:"auto",paddingLeft:"0.5rem",paddingRight:"0.5rem"}}>
                ...
                ...(isExtraSmallScreen && styles.containerextrasmall),
                    ...
                    <Box>
                        ...
                        <Text  style={{fontSize: "20px",zIndex:-1}}>Pos Acessórios</Text>   
                        </Box>
                    </Box>     
                    <Text style={{
                        ...styles.textdefault3,
                        ...
                        <span style={{fontWeight:"bold"}}>Pos Acessórios</span>
                    </Text>
                    <Text style={{
                        ...styles.textdefault,
                        maxWidth: "1800px",
                        ...
                        Veja os pos acessórios disponíveis na loja
                    </Text>
                </Box>
                <Box sx={{...styles.containermain, 
                    ...
                    ...(isExtraSmallScreen && styles.container1extrasmall)}}>
                        {posacessorios.length>0  &&(
                                itemsToShow.map((posacessorios, index) => (
                                        <Box sx={styles.viewcontainer} onClick = {(e) =>handleproduct(posacessorios)}>
                                        <Box sx={styles.containerfeaturesmainproduct}> 
                                            ...
                                            <Box sx={styles.containerfeatures}>
                                                ...
                                                <Text style={[styles.textdefault,{fontSize:"13px"}]}>Ref: {posacessorios[2]} </Text>    
                                                ...
                                                <Text style={styles.textdefault2}>
                                                    <span style={{color:"black"}}>{posacessorios[5]} €</span> 
                                                </Text>
                                            </Box>
                                        </Box>
                                        </Box> 
                        ...
                        <Box sx={styles.pages}>
                            <Box sx={styles.pagesflex}>
                                {parseInt(pageNumber, 10) <= maxpages && parseInt(pageNumber, 10) > 1 && (
                                    <Link  to={`/posacessórios?page=${parseInt(pageNumber, 10) - 1}`} id='aheader' >
