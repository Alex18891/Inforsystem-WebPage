...
export default function Memoriasusb() {
    ...
    const [memoriasusb, setmemoriasusb] = useState([]);
    const [memoriasusbfilter, setmemoriasusbfilter] = useState([]);
    const [maxpages, setmaxpages] = useState([]);
    const [maxpagesfilter, setmaxpagesfilter] = useState([]);
    const [marcamemoriasusb, setmarcamemoriasusb] = useState([]);
    const [checkboxmarca,setcheckboxmarca] = useState(Array(marcamemoriasusb.length).fill(false));
    const pageNumber = queryParams.get("page");
    const itemsToShow = memoriasusb.slice(((parseInt(pageNumber, 10) ) - 1) * itemsPerPage, (parseInt(pageNumber, 10) ) * itemsPerPage);
    ...
    const renderLinks = () => {
        ...
        for (let i = 0; i < maxpages; i++) {
          ...
          if (currentNumber <= maxpages && currentNumber <= 7) {
            elements.push(
              <Link to={`/memóriasusb?page=${currentNumber}`} id='aheader' key={currentNumber}>
                {currentNumber}
                <Text>&nbsp; | </Text>
              </Link>
            );
    ...
    const readFile = async () => {
        ...
        reader.onload = (e) => {
            ...
            if (jsonData && jsonData.length > 0) {
                ...
                const combinedmarcaarray =Array.from(new Set(memoriasusb.map(value => value[0]))) 
                setmarcamemoriasusb(combinedmarcaarray)
                const maxPages = Math.ceil(memoriasusb.length / itemsPerPage);
                setmaxpages(maxPages)
                setmemoriasusb(memoriasusb)
                setmemoriasusbfilter(memoriasusb)
    ...
    useEffect(()=>{       
        const filterBySelectedCheckboxes = () => {
            return memoriasusbfilter.filter(item=>{      //Filter the pcs by family     
                for(let i = 0; i<checkboxmarca.length;i++)//For that runs up to all the checkboxs
                {
                    console.log( marcamemoriasusb[i])
                    if(checkboxmarca[i] && item[0] ==  marcamemoriasusb[i])//If the checkbox is selecte and element 1 of pcsfilter array(family) is equal to the familypcs array return true
                    {    
                        return true;
                    }   
                }
        ...
        if(deduplicated.length>0)
        {
            ...
            setmaxpages(maxPages);
            setmemoriasusb(deduplicated);
            navigate('?page=1');
        }
        else{
            setmemoriasusb(memoriasusbfilter);
            setmaxpages(maxpagesfilter);
            navigate('?page=1');
        }
      
    },[checkboxmarca, marcamemoriasusb, memoriasusbfilter])
            ...
            <Box sx={{maxWidth:"1800px",  margin:"auto",paddingLeft:"0.5rem",paddingRight:"0.5rem"}}>
                ...
                ...(isExtraSmallScreen && styles.containerextrasmall),
                    ...
                    <Text style={{
                        ...
                        <span style={{fontWeight:"bold"}}>Memórias USB</span>
                    </Text>
                    <Text style={{
                        ...styles.textdefault,
                        maxWidth: "1800px",
                        ...
                        Veja as memórias usb disponíveis na loja
                    </Text>
                </Box>
                                ...
                                itemsToShow.map((memoriasusb, index) => (
                                        ...
                                        <Box sx={styles.containerfeaturesmainproduct}> 
                                            ...
                                            <Box sx={styles.containerfeatures}>
                                                ...
                                                <Text style={[styles.textdefault,{fontSize:"13px"}]}>Ref: {memoriasusb[2]} </Text>    
                                                ...
                                                <Text style={styles.textdefault2}>
                                                    <span style={{color:"black"}}>{memoriasusb[5]} €</span> 
                                                </Text>
                                            </Box>
                                        </Box>
                                        </Box> 
                        ...
                        <Box sx={styles.pages}>
                            <Box sx={styles.pagesflex}>      
                                {parseInt(pageNumber, 10) <= maxpages && parseInt(pageNumber, 10) > 1 && (
                                    <Link  to={`/memóriasusb?page=${parseInt(pageNumber, 10) - 1}`} id='aheader' >
