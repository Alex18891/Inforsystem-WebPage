...
export default function DiscosHDD() {
    ...
    const [discoshdd, setdiscoshdd] = useState([]);
    const [discoshddfilter, setdiscoshddfilter] = useState([]);
    const [maxpages, setmaxpages] = useState([]);
    const [maxpagesfilter, setmaxpagesfilter] = useState([]);
    const [marcadiscoshdd, setmarcadiscoshdd] = useState([]);
    const [checkboxmarca,setcheckboxmarca] = useState(Array(marcadiscoshdd.length).fill(false));
    const pageNumber = queryParams.get("page");
    const itemsToShow = discoshdd.slice(((parseInt(pageNumber, 10) ) - 1) * itemsPerPage, (parseInt(pageNumber, 10) ) * itemsPerPage);
    ...
    const renderLinks = () => {
        ...
        for (let i = 0; i < maxpages; i++) {
          ...
          if (currentNumber <= maxpages && currentNumber <= 7) {
            elements.push(
              <Link to={`/discosHDD?page=${currentNumber}`} id='aheader' key={currentNumber}>
                {currentNumber}
                <Text>&nbsp; | </Text>
              </Link>
            );
    ...
    const readFile = async () => {
        ...
        reader.onload = (e) => {
            ...
            if (jsonData && jsonData.length > 0) {
                ...
                const combinedmarcaarray =Array.from(new Set(discoshdd.map(value => value[0]))) 
                setmarcadiscoshdd(combinedmarcaarray)
                const maxPages = Math.ceil(discoshdd.length / itemsPerPage);
                setmaxpages(maxPages)
                setdiscoshdd(discoshdd)
                setdiscoshddfilter(discoshdd)
    ...
    useEffect(()=>{       
        const filterBySelectedCheckboxes = () => {
            return discoshddfilter.filter(item=>{      //Filter the pcs by family     
                for(let i = 0; i<checkboxmarca.length;i++)//For that runs up to all the checkboxs
                {
                    console.log( marcadiscoshdd[i])
                    if(checkboxmarca[i] && item[0] ==  marcadiscoshdd[i])//If the checkbox is selecte and element 1 of pcsfilter array(family) is equal to the familypcs array return true
                    {    
                        return true;
                    }   
                }
        ...
        if(deduplicated.length>0)
        {
            ...
            setmaxpages(maxPages);
            setdiscoshdd(deduplicated);
            navigate('?page=1');
        }
        else{
            setdiscoshdd(discoshddfilter);
            setmaxpages(maxpagesfilter);
            navigate('?page=1');
        }
      
    },[checkboxmarca, marcadiscoshdd, discoshddfilter])
            ...
            <Box sx={{maxWidth:"1800px",  margin:"auto",paddingLeft:"0.5rem",paddingRight:"0.5rem"}}>
                ...
                ...(isExtraSmallScreen && styles.containerextrasmall),
                    ...
                    <Box>
                        ...
                        <Text  style={{fontSize: "20px",zIndex:-1}}>Discos HDD</Text>   
                        </Box>
                    </Box>     
                    <Text style={{
                        ...styles.textdefault3,
                        ...
                        <span style={{fontWeight:"bold"}}>Discos HDD</span>
                    </Text>
                    <Text style={{
                        ...styles.textdefault,
                        maxWidth: "1800px",
                        ...
                        Veja os discos hdd disponíveis na loja
                    </Text>
                </Box>
                <Box sx={{...styles.containermain, 
                    ...
                    ...(isExtraSmallScreen && styles.container1extrasmall)}}>
                        {discoshdd.length>0  &&(
                                itemsToShow.map((discoshdd, index) => (
                                        <Box sx={styles.viewcontainer} onClick = {(e) =>handleproduct(discoshdd)}>
                                        <Box sx={styles.containerfeaturesmainproduct}> 
                                            ...
                                            <Box sx={styles.containerfeatures}>
                                                ...
                                                <Text style={[styles.textdefault,{fontSize:"13px"}]}>Ref: {discoshdd[2]} </Text>    
                                                ...
                                                <Text style={styles.textdefault2}>
                                                    <span style={{color:"black"}}>{discoshdd[5]} €</span> 
                                                </Text>
                                            </Box>
                                        </Box>
                                        </Box> 
                        ...
                        <Box sx={styles.pages}>
                            <Box sx={styles.pagesflex}>
                                {parseInt(pageNumber, 10) <= maxpages && parseInt(pageNumber, 10) > 1 && (
                                    <Link  to={`/discosHDD?page=${parseInt(pageNumber, 10) - 1}`} id='aheader' >
