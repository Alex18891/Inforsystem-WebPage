...
export default function Memoriascartoes() {
    ...
    const [memoriascartoes, setmemoriascartoes] = useState([]);
    const [memoriascartoesfilter, setmemoriascartoesfilter] = useState([]);
    const [maxpages, setmaxpages] = useState([]);
    const [maxpagesfilter, setmaxpagesfilter] = useState([]);
    const [marcamemoriascartoes, setmarcamemoriascartoes] = useState([]);
    const [checkboxmarca,setcheckboxmarca] = useState(Array(marcamemoriascartoes.length).fill(false));
    const pageNumber = queryParams.get("page");
    const itemsToShow = memoriascartoes.slice(((parseInt(pageNumber, 10) ) - 1) * itemsPerPage, (parseInt(pageNumber, 10) ) * itemsPerPage);
    ...
    const renderLinks = () => {
        ...
        for (let i = 0; i < maxpages; i++) {
          ...
          if (currentNumber <= maxpages && currentNumber <= 7) {
            elements.push(
              <Link to={`/memóriascartões?page=${currentNumber}`} id='aheader' key={currentNumber}>
                {currentNumber}
                <Text>&nbsp; | </Text>
              </Link>
            );
    ...
    const readFile = async () => {
        ...
        reader.onload = (e) => {
            ...
            if (jsonData && jsonData.length > 0) {
                ...
                const combinedmarcaarray =Array.from(new Set(memoriascartoes.map(value => value[0]))) 
                setmarcamemoriascartoes(combinedmarcaarray)
                const maxPages = Math.ceil(memoriascartoes.length / itemsPerPage);
                setmaxpages(maxPages)
                setmemoriascartoes(memoriascartoes)
                setmemoriascartoesfilter(memoriascartoes)
    ...
    useEffect(()=>{       
        const filterBySelectedCheckboxes = () => {
            return memoriascartoesfilter.filter(item=>{      //Filter the pcs by family     
                for(let i = 0; i<checkboxmarca.length;i++)//For that runs up to all the checkboxs
                {
                    console.log( marcamemoriascartoes[i])
                    if(checkboxmarca[i] && item[0] ==  marcamemoriascartoes[i])//If the checkbox is selecte and element 1 of pcsfilter array(family) is equal to the familypcs array return true
                    {    
                        return true;
                    }   
                }
        ...
        if(deduplicated.length>0)
        {
            ...
            setmaxpages(maxPages);
            setmemoriascartoes(deduplicated);
            navigate('?page=1');
        }
        else{
            setmemoriascartoes(memoriascartoesfilter);
            setmaxpages(maxpagesfilter);
            navigate('?page=1');
        }
      
    },[checkboxmarca, marcamemoriascartoes, memoriascartoesfilter])
        ...
        <Box  sx={styles.container1}>
            <Box sx={[styles.viewcontainer,{paddingLeft:"0"}]}>      
                ...
                <Box sx={styles.containerfeatures}>
                    {
                        memoriascartoes.length>0  &&(
                            marcamemoriascartoes.map((pc, index) => (
            ...
            <Box sx={{maxWidth:"1800px",  margin:"auto",paddingLeft:"0.5rem",paddingRight:"0.5rem"}}>
                ...
                ...(isExtraSmallScreen && styles.containerextrasmall),
                    ...
                    <Box>
                        ...
                        <Text  style={{fontSize: "20px",zIndex:-1}}>Memórias USB</Text>   
                        </Box>
                    </Box>     
                    <Text style={{
                        ...styles.textdefault3,
                        ...
                        <span style={{fontWeight:"bold"}}>Memórias Cartões</span>
                    </Text>
                    <Text style={{
                        ...styles.textdefault,
                        maxWidth: "1800px",
                        ...
                        Veja as memórias cartões disponíveis na loja
                    </Text>
                </Box>
                <Box sx={{...styles.containermain, 
                    ...
                    ...(isExtraSmallScreen && styles.container1extrasmall)}}>
                        {memoriascartoes.length>0  &&(
                                itemsToShow.map((memoriascartoes, index) => (
                                        <Box sx={styles.viewcontainer} onClick = {(e) =>handleproduct(memoriascartoes)}>
                                        <Box sx={styles.containerfeaturesmainproduct}> 
                                            ...
                                            <Box sx={styles.containerfeatures}>
                                                ...
                                                <Text style={[styles.textdefault,{fontSize:"13px"}]}>Ref: {memoriascartoes[2]} </Text>    
                                                ...
                                                <Text style={styles.textdefault2}>
                                                    <span style={{color:"black"}}>{memoriascartoes[5]} €</span> 
                                                </Text>
                                            </Box>
                                        </Box>
                                        </Box> 
                        ...
                        <Box sx={styles.pages}>
                            <Box sx={styles.pagesflex}>      
                                {parseInt(pageNumber, 10) <= maxpages && parseInt(pageNumber, 10) > 1 && (
                                    <Link  to={`/memóriascartões?page=${parseInt(pageNumber, 10) - 1}`} id='aheader' >
