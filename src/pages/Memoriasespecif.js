...
export default function Memoriasespecif() {
    ...
    const [memoriasespecif, setmemoriasespecif] = useState([]);
    const [memoriasespeciffilter, setmemoriasespeciffilter] = useState([]);
    const [maxpages, setmaxpages] = useState([]);
    const [maxpagesfilter, setmaxpagesfilter] = useState([]);
    const [marcamemoriasespecif, setmarcamemoriasespecif] = useState([]);
    const [checkboxmarca,setcheckboxmarca] = useState(Array(marcamemoriasespecif.length).fill(false));
    const pageNumber = queryParams.get("page");
    const itemsToShow = memoriasespecif.slice(((parseInt(pageNumber, 10) ) - 1) * itemsPerPage, (parseInt(pageNumber, 10) ) * itemsPerPage);
    ...
    const renderLinks = () => {
        ...
        for (let i = 0; i < maxpages; i++) {
          ...
          if (currentNumber <= maxpages && currentNumber <= 7) {
            elements.push(
              <Link to={`/memóriasespecíficas?page=${currentNumber}`} id='aheader' key={currentNumber}>
                {currentNumber}
                <Text>&nbsp; | </Text>
              </Link>
            );
    ...
    const readFile = async () => {
        ...
        reader.onload = (e) => {
            ...
            if (jsonData && jsonData.length > 0) {
                ...
                const combinedmarcaarray =Array.from(new Set(memoriasespecificias.map(value => value[0]))) 
                setmarcamemoriasespecif(combinedmarcaarray)
                const maxPages = Math.ceil(memoriasespecificias.length / itemsPerPage);
                setmaxpages(maxPages)
                setmemoriasespecif(memoriasespecificias)
                setmemoriasespeciffilter(memoriasespecificias)
    ...
    useEffect(()=>{       
        const filterBySelectedCheckboxes = () => {
            return memoriasespeciffilter.filter(item=>{      //Filter the pcs by family     
                for(let i = 0; i<checkboxmarca.length;i++)//For that runs up to all the checkboxs
                {
                    console.log( marcamemoriasespecif[i])
                    if(checkboxmarca[i] && item[0] ==  marcamemoriasespecif[i])//If the checkbox is selecte and element 1 of pcsfilter array(family) is equal to the familypcs array return true
                    {    
                        return true;
                    }   
                }
        ...
        if(deduplicated.length>0)
        {
            ...
            setmaxpages(maxPages);
            setmemoriasespecif(deduplicated);
            navigate('?page=1');
        }
        else{
            setmemoriasespecif(memoriasespeciffilter);
            setmaxpages(maxpagesfilter);
            navigate('?page=1');
        }
      
    },[checkboxmarca, marcamemoriasespecif, memoriasespeciffilter])
        ...
        <Box  sx={styles.container1}>
            <Box sx={[styles.viewcontainer,{paddingLeft:"0"}]}>      
                ...
                <Box sx={styles.containerfeatures}>
                    {
                        memoriasespecif.length>0  &&(
                            marcamemoriasespecif.map((pc, index) => (
            ...
            <Box sx={{maxWidth:"1800px",  margin:"auto",paddingLeft:"0.5rem",paddingRight:"0.5rem"}}>
                ...
                ...(isExtraSmallScreen && styles.containerextrasmall),
                    ...
                    <Box>
                        ...
                        <Text  style={{fontSize: "20px",zIndex:-1}}>Memórias Específicas</Text>   
                        </Box>
                    </Box>     
                    <Text style={{
                        ...styles.textdefault3,
                        ...
                        <span style={{fontWeight:"bold"}}>Memórias Específicas</span>
                    </Text>
                    <Text style={{
                        ...styles.textdefault,
                        maxWidth: "1800px",
                        ...
                        Veja as memórias específicas disponíveis na loja
                    </Text>
                </Box>
                <Box sx={{...styles.containermain, 
                    ...
                    ...(isExtraSmallScreen && styles.container1extrasmall)}}>
                        {memoriasespecif.length>0  &&(
                                itemsToShow.map((memoriasespecif, index) => (
                                        <Box sx={styles.viewcontainer} onClick = {(e) =>handleproduct(memoriasespecif)}>
                                        <Box sx={styles.containerfeaturesmainproduct}> 
                                            ...
                                            <Box sx={styles.containerfeatures}>
                                                ...
                                                <Text style={[styles.textdefault,{fontSize:"13px"}]}>Ref: {memoriasespecif[2]} </Text>    
                                                ...
                                                <Text style={styles.textdefault2}>
                                                    <span style={{color:"black"}}>{memoriasespecif[5]} €</span> 
                                                </Text>
                                            </Box>
                                        </Box>
                                        </Box> 
                        ...
                        <Box sx={styles.pages}>
                            <Box sx={styles.pagesflex}>      
                                {parseInt(pageNumber, 10) <= maxpages && parseInt(pageNumber, 10) > 1 && (
                                    <Link  to={`/memóriasespecíficas?page=${parseInt(pageNumber, 10) - 1}`} id='aheader' >
